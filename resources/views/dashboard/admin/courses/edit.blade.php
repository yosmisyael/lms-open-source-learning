...
<div class="row">
    <div class="col-lg-8 mb-3">
        <form action="/admin/dashboard/courses/{{ $data->id }}" method="post" enctype="multipart/form-data">
            @method('put')
            @csrf
            {{-- old image --}}
            ...
            <div class="form-outline form-floating mb-4">
              <input type="text" id="total_lessons" name="total_lessons" class="form-control form-control-lg @error('total_lessons')
                is-invalid
              @enderror"
                placeholder="Enter lessons total for this course" required required value="{{ old('total_lessons', $data->total_lessons) }}" />
              ...
              <select id="test" class="form-select" aria-label="Default select example" name="test_id">     
                @if ($tests->count())
                  @foreach ($tests as $test)
                    @if (old('test_id', $data->test_id) == $test->id)
            ...
            <div class="mb-3">
              ...
              <select id="level" class="form-select" aria-label="Default select example" name="level">     
                <option value="beginner" @if ($data->level == 'beginner')
                  selected
                @endif>Beginner</option>                        
                <option value="advanced" @if ($data->level == 'advanced')
                  selected
